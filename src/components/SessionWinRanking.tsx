import { StyleSheet, Text, View } from 'react-native';
import { useTheme, useThemedStyles, type AppTheme } from '../theme';
import { SessionWinEntry } from '../utils/session';
import { PlayerAvatar } from './PlayerAvatar';

type Props = {
  ranking: SessionWinEntry[];
};

const MEDAL = ['🥇', '🥈', '🥉'];

export function SessionWinRanking({ ranking }: Props) {
  const theme = useTheme();
  const styles = useThemedStyles(createStyles);

  const withWins = ranking.filter((e) => e.wins > 0);
  if (withWins.length === 0) {
    return (
      <View style={styles.wrap}>
        <Text style={styles.heading}>Victorias</Text>
        <Text style={styles.empty}>
          Aún no hay partidas finalizadas en esta sesión.
        </Text>
      </View>
    );
  }

  const podiumOrder =
    withWins.length >= 3
      ? [withWins[1], withWins[0], withWins[2]]
      : withWins.length === 2
        ? [withWins[1], withWins[0]]
        : [withWins[0]];

  const podiumHeights =
    withWins.length >= 3
      ? [72, 96, 56]
      : withWins.length === 2
        ? [72, 96]
        : [88];

  return (
    <View style={styles.wrap}>
      <Text style={styles.heading}>Quién gana más</Text>

      <View style={styles.podiumRow}>
        {podiumOrder.map((entry, index) => {
          const place =
            withWins.length >= 3
              ? index === 0
                ? 2
                : index === 1
                  ? 1
                  : 3
              : withWins.length === 2
                ? index === 0
                  ? 2
                  : 1
                : 1;
          const height = podiumHeights[index];

          return (
            <View key={entry.player.id} style={styles.podiumCol}>
              <Text style={styles.medal}>
                {place <= 3 ? MEDAL[place - 1] : `#${place}`}
              </Text>
              <View
                style={[
                  styles.pedestal,
                  { height, borderColor: entry.player.color + '88' },
                  place === 1 && styles.pedestalFirst,
                ]}
              >
                <Text style={styles.pedestalScore}>{entry.wins}</Text>
                <Text style={styles.pedestalPts}>
                  {entry.wins === 1 ? 'victoria' : 'victorias'}
                </Text>
              </View>
              <PlayerAvatar
                name={entry.player.name}
                color={entry.player.color}
                avatar={entry.player.avatar}
                size={36}
                radius={10}
              />
              <Text style={styles.podiumName} numberOfLines={1}>
                {entry.player.name}
              </Text>
            </View>
          );
        })}
      </View>

      <View style={styles.list}>
        <Text style={styles.listTitle}>Clasificación por victorias</Text>
        {ranking.map((entry) => (
          <View key={entry.player.id} style={styles.listRow}>
            <Text style={styles.listRank}>#{entry.rank}</Text>
            <PlayerAvatar
              name={entry.player.name}
              color={entry.player.color}
              avatar={entry.player.avatar}
              size={28}
              radius={8}
            />
            <Text style={styles.listName} numberOfLines={1}>
              {entry.player.name}
            </Text>
            <Text style={styles.listScore}>
              {entry.wins} {entry.wins === 1 ? 'victoria' : 'victorias'}
            </Text>
          </View>
        ))}
      </View>
    </View>
  );
}

const createStyles = (theme: AppTheme) => StyleSheet.create({
  wrap: {
    backgroundColor: theme.surface,
    borderRadius: 16,
    borderWidth: 1,
    borderColor: theme.border,
    padding: 16,
    gap: 16,
  },
  heading: {
    fontSize: 18,
    fontWeight: '800',
    color: theme.text,
    textAlign: 'center',
  },
  empty: {
    fontSize: 14,
    color: theme.textMuted,
    textAlign: 'center',
    lineHeight: 20,
  },
  podiumRow: {
    flexDirection: 'row',
    alignItems: 'flex-end',
    justifyContent: 'center',
    gap: 10,
    paddingTop: 8,
  },
  podiumCol: {
    flex: 1,
    alignItems: 'center',
    gap: 6,
    maxWidth: 110,
  },
  medal: {
    fontSize: 22,
  },
  pedestal: {
    width: '100%',
    borderRadius: 12,
    borderWidth: 2,
    backgroundColor: theme.surfaceLight,
    alignItems: 'center',
    justifyContent: 'center',
    gap: 0,
    paddingVertical: 4,
  },
  pedestalFirst: {
    backgroundColor: theme.accent + '18',
  },
  pedestalScore: {
    fontSize: 22,
    fontWeight: '800',
    color: theme.text,
  },
  pedestalPts: {
    fontSize: 10,
    color: theme.textMuted,
    fontWeight: '600',
    textAlign: 'center',
  },
  avatar: {
    width: 36,
    height: 36,
    borderRadius: 10,
    alignItems: 'center',
    justifyContent: 'center',
  },
  avatarText: {
    fontSize: 16,
    fontWeight: '800',
    color: '#fff',
  },
  podiumName: {
    fontSize: 13,
    fontWeight: '700',
    color: theme.text,
    textAlign: 'center',
  },
  list: {
    gap: 8,
    paddingTop: 4,
    borderTopWidth: 1,
    borderTopColor: theme.border,
  },
  listTitle: {
    fontSize: 13,
    fontWeight: '700',
    color: theme.textMuted,
    textTransform: 'uppercase',
    letterSpacing: 0.4,
  },
  listRow: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 10,
    paddingVertical: 6,
  },
  listRank: {
    width: 28,
    fontSize: 14,
    fontWeight: '800',
    color: theme.textMuted,
  },
  listDot: {
    width: 10,
    height: 10,
    borderRadius: 5,
  },
  listName: {
    flex: 1,
    fontSize: 15,
    fontWeight: '600',
    color: theme.text,
  },
  listScore: {
    fontSize: 14,
    fontWeight: '700',
    color: theme.accent,
  },
});
